...
class ReaderElementTest extends TestCase
{
    ...
    public function testAttributeQuoting(): void
    {
        ...
        $this->assertEquals($str, $dom->find('div', 0)->outertext);
        $dom->clear();
    ...
    public function testInnertext(): void
    {
        ...
        $this->assertEquals('<html><head></head><body><br><span>bar</span></body></html>', (string) $dom);

        // Set head innertext
        $dom->find('head', 0)->innertext = 'ok';
        $this->assertEquals('<html><head>ok</head><body><br><span>bar</span></body></html>', (string) $dom);
    ...
    public function testOutertextTable(): void
    {
        ...
        $this->assertEquals('<tr><th>Head1</th><th>Head2</th><th>Head3</th></tr>', $dom->find('tr', 0)->outertext);
        $this->assertEquals('<tr><td>1</td><td>2</td><td>3</td></tr>', $dom->find('tr', 1)->outertext);
        $dom->clear();
        ...
        // List test
        $dom = str_get_html('<ul><li><b>li11</b></li><li><b>li12</b></li></ul><ul><li><b>li21</b></li><li><b>li22</b></li></ul>');
        $this->assertEquals('<ul><li><b>li11</b></li><li><b>li12</b></li></ul>', $dom->find('ul', 0)->outertext);
        $this->assertEquals('<ul><li><b>li21</b></li><li><b>li22</b></li></ul>', $dom->find('ul', 1)->outertext);
        $dom->clear();
    }
    ...
    public function testReplacement(): void
    {
        ...
        $dom = str_get_html('<select name=something><options>blah</options><options>blah2</options></select>');
    ...
    public function testParagraphHandling(): void
    {
        ...
        $this->assertEquals('ok0link0', $dom->find('p', 0)->plaintext);
        $this->assertEquals('ok1link1', $dom->find('p', 1)->plaintext);
        $this->assertEquals('ok2link2', $dom->find('p', 2)->plaintext);
        ...
        $this->assertEquals('link0', $dom->find('p a', 0)->plaintext);
        $this->assertEquals('link1', $dom->find('p a', 1)->plaintext);
        $this->assertEquals('link2', $dom->find('p a', 2)->plaintext);
    ...
    public function testCodeTag(): void
    {
        ...
        HTML;
        $dom = str_get_html($str, true, true, 'UTF-8', false);
        $this->assertCount(1, $dom->find('code'));
        $dom->clear();
    }
}
